...
from dream_geo.utilities import exists_or_mkdir
import dream_geo.analysis as dream_analysis
...
def train_network(args):
    ...
    train_name = '{0}_{1}_lr_{2}_bs_{3}_seed_{4}_epoch_{5}_try_{6}'.format(args.type, args.owner, args.learning_rate,
                                                                           args.batch_size, args.random_seed,
                                                                           args.epochs, args.times)
    if not args.output_dir:
        args.output_dir = '/mnt/all_ckpt/{0}/{1}/'.format(args.owner, args.type) + train_name
    ...
    tb_path = '/mnt/all_logs/{0}/{1}/'.format(args.owner, args.type) + train_name + '/logs/tb'
    exists_or_mkdir(tb_path)
    writer = SummaryWriter(tb_path)
    infer_output_dir = '/mnt/all_inference/{0}/{1}/'.format(args.owner, args.type) + train_name
...
#            print(sample["config"]["data_path"])
#            print(sample["config"]["name"])
            loss = dream_network.train(network_input_heads,training_labels,this_epoch, sample["config"]["name"])
        ...
        with torch.no_grad():
            ...
            for valid_batch_idx, valid_sample in enumerate(tqdm(valid_data_loader)):
                ...
                valid_network_input_heads.append(sample["keypoint_positions"].cuda())
                if (
