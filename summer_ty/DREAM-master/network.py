...
class DreamNetwork:
    ...
    def train(self, network_input_heads,target,this_epoch, pose_gt,camera_mat):
        assert self.optimizer, "Optimizer must be defined. Use enable_training() first."

        self.optimizer.zero_grad()

        loss,loss1,loss2 = self.loss(network_input_heads, target,pose_gt,camera_mat,this_epoch)
    ...
    def loss(self, network_input_heads, target,gt_pose, camera_mat,this_epoch):
        camera_mat = torch.from_numpy(camera_mat)
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        network_output_heads = self.model(network_input_heads[0])
        x2d = network_output_heads[1].double().to(device)
        x2d_gt = network_input_heads[2].double().to(device)
        x2d_gt[:, :, 0] = x2d_gt[:, :, 0] / 100 * 640
        x2d_gt[:, :, 1] = x2d_gt[:, :, 1] / 100 * 480
        x2d[:, :, 0] = x2d[:, :, 0] / 100 * 640
        x2d[:, :, 1] = x2d[:, :, 1] / 100 * 480
        x3d = network_input_heads[1]
        P_out = self.bpnp(x2d,x3d,camera_mat)
        x2d_pj = batch_project_m3d(P_out, x3d, camera_mat)
        loss_2 = ((x2d_pj - x2d_gt) ** 2).mean() + ((x2d_pj - x2d) ** 2).mean() + ((x2d_gt - x2d) ** 2).mean()
    ...
    def inference(self, network_input, network_input_x3d = torch.zeros(1,7,3)):
