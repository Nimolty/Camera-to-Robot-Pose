...
class DLASegCA(BaseModelCA):
    def __init__(self, num_layers, heads, head_convs, opt):
        super(DLASegCA, self).__init__(
            heads, head_convs, 1, 64 if num_layers == 34 else 256, opt=opt)
        down_ratio=4
        self.opt = opt
        self.node_type = DLA_NODE[opt.dla_node]
        # print('Using node type:', self.node_type)
        ...
        self.base = globals()['dla{}'.format(num_layers)](
            pretrained=(opt.load_model == ''), opt=opt)
        self.hm_base = globals()['dla{}'.format(num_layers)](
            pretrained=(opt.load_model == ''), opt=opt)
    ...
    def imgpre2feats(self, x, pre_img=None, pre_hm=None):
        ...
        x_up = self.dla_up(x)
        for i in range(self.last_level - self.first_level):
            y.append(x_up[i].clone())
        self.ida_up(y, 0, len(y))
        
        img_pre = []
        pre_img = self.base(pre_img)
        pre_img_up = self.dla_up(pre_img)
        for i in range(self.last_level - self.first_level):
            img_pre.append(pre_img_up[i].clone())
        ...
        # pre_h = self.dla_up(pre_h)
        hm_pre = []
        for i in range(self.last_level - self.first_level):
            hm_pre.append(pre_h[i].clone())
        self.hm_ida_up(hm_pre, 0, len(hm_pre))
        # 此时的y_pre_h是一个list,里面有3个BXCXHXW的features
        
#        for idx, y_ind in enumerate(x):
#            print(f"{idx}的shape为{y_ind.shape}") 
        
        # 此时我们得到的每个features的size为B x C x H x W (测试为16 x 64 x 120 x 120)
        # 现在我们fusion前后两个frame的最后一个feature
        
        fusion_features = Two_Frames_Fusion(y[-1], img_pre[-1], self.opt) # B X C X H X W
