...
def generic_decode(output, K=7, opt=None):
  if not ('hm' in output):
    return {}
...
def dream_generic_decode(output, K=7, opt=None):
  ...
  # heat = _nms(heat)
  # scores, inds, clses, ys0, xs0 = _softargmaxpavlo(heat)
  # _peaks_info(heat)
  # scores, inds, clses, ys0, xs0 = _topk(heat, K=K)
  ...
  # print('inds', inds)
  # print('clses', clses.shape)
  # print('ys0.shape', ys0.shape)
  # print('xs0.shape', xs0.shape)
  ...
  cts_wreg = torch.cat([xs.unsqueeze(2), ys.unsqueeze(2)], dim=2) #用来存储加入reg后的cts
  ret["cts_wreg"] = cts_wreg
  ret["regs"] = cts_wreg - torch.cat([(xs0.view(batch, K, 1)).unsqueeze(2),  (ys0.view(batch, K, 1)).unsqueeze(2)], dim=2 )
  # print('cts_wreg', cts_wreg)
