...
class ModleWithLoss(torch.nn.Module):
  ...
  def forward(self, batch):
    pre_img = batch['prev_image_rgb_input'].to(self.opt.device) if 'prev_image_rgb_input' in batch else None
    pre_hm = batch["prev_belief_maps"].to(self.opt.device)[:, 0, :, :] if "prev_belief_maps" in batch else None
    pre_hm = pre_hm.unsqueeze(1)
    print(pre_hm.shape)
    outputs = self.model(batch['next_image_rgb_input'].to(self.opt.device), pre_img, pre_hm)
    print(len(outputs))
