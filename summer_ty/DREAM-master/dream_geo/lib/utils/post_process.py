...
import numpy as np
import cv2
from .image import transform_preds_with_trans, get_affine_transform, transform_preds_with_trans_offset
...
def dream_generic_post_process(
  opt, dets, c, s, h, w, num_classes, calibs=None, height=-1, width=-1):
  if not ('scores' in dets):
    return [{}], [{}]
  ret = []
  ...
  for i in range(len(dets['scores'])):
    ...
    for j in range(len(dets['scores'][i])):
      ...
      item['ct'] = transform_preds_with_trans(
        (dets['cts'][i][j]).reshape(1, 2), trans).reshape(2)
      item["ct_wreg"] = transform_preds_with_trans(
        (dets["cts_wreg"][i][j]).reshape(1, 2), trans).reshape(2)
      item["reg"] = transform_preds_with_trans_offset(
        (dets["regs"][i][j]).reshape(1, 2), trans).reshape(2)
      item["ct_wreg_output"] = dets["cts_wreg"][i][j].reshape(2)
      item["reg_output"] = dets["regs"][i][j].reshape(2)
      item["ct_output"] = dets["cts"][i][j].reshape(2)
      # print('inv_transform' , trans)
      # print('cr_wreg', item["ct_wreg"])
      # print('reg', item["ct_wreg_output"] - item["ct_output"]) # 数值太小，基本都在1e-3或者1e-4量级，问题很大
      ...
      if 'bboxes' in dets:
        # print("bboxes i j", dets["bboxes"][i][j])
        bbox = transform_preds_with_trans(
          dets['bboxes'][i][j].reshape(2, 2), trans).reshape(4)
        # print('bb0x', bbox)
        item['bbox'] = bbox
