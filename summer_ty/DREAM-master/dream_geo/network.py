...
class DreamNetwork:
    ...
    def train(self, network_input_heads,target,this_epoch, train_flag):
        assert self.optimizer, "Optimizer must be defined. Use enable_training() first."

        self.optimizer.zero_grad()

        loss = self.loss(network_input_heads, target, train_flag)
        print(loss)
        loss.backward()
        self.optimizer.step()
    ...
    def loss(self, network_input_heads, target,train_flag):
        network_output_heads = self.model(network_input_heads[0],network_input_heads[1])
    ...
    def inference(self, network_input, network_input_x3d = torch.zeros(1,7,3), network_name = '0'):

        if self.architecture_type == "dream_legacy":
            network_head_outputs = self.model(network_input,network_input_x3d)
            ...
            network_output = self.model(network_input,network_input_x3d)

        elif self.network_config["architecture"]["output_heads"] == ["belief_maps"]:

            for i in range(network_input_x3d.shape[0]):
                x3d0 = network_input_x3d[i][0].clone()
                for j in range(network_input_x3d.shape[1]):
                    network_input_x3d[i][j] -= x3d0

            network_input_x3d*= 100
            device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            RotateX = torch.from_numpy(np.array([[1, 0, 0], [0, 0, 1], [0, -1, 0]]).reshape(3,3)).float().to(device)
            RotateZ = torch.from_numpy(np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]]).reshape(3,3)).float().to(device)
            for i in range(network_input_x3d.shape[0]):
                for j in range(network_input_x3d.shape[1]):
                    network_input_x3d[i][j] = RotateZ @ RotateX @  network_input_x3d[i][j]

            network_head_output = self.model(network_input,network_input_x3d)
