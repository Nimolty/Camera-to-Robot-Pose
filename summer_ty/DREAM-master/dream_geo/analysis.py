...
def analyze_ndds_center_dream_dataset(
    ...
    with torch.no_grad():
        ...
        for idx, sample in enumerate(zip(json_list, detected_kp_proj_list)):
            ...
            # 专门生成新图片，然后存储
#            print('this_json_path', this_json_path)
#            this_raw_img_path = this_json_path.replace(".json", ".rgb.jpg")
#            this_raw_img_save_path = this_json_path.split('/')[-1].replace(".json", ".png")
#            raw_img = PILImage.open(this_raw_img_path)
#            
#            kp_imgs = []
#            for idx, kp_name in enumerate(keypoint_names):
#                gt_kp_raw = object_keypoints[idx]["projected_location"]
#                detected_kp_raw = this_detected_kps_raw.tolist()[idx]
#                kp_img = dream.image_proc.overlay_points_on_image(
#                raw_img,
#                [gt_kp_raw, detected_kp_raw],
#                [keypoint_names[idx]] * 2,
...
#            
#            mosaic_images = dream.image_proc.mosaic_images(
...
#            mosaic_images.save(
#            os.path.join(mosaic_path, this_raw_img_save_path)
#            )
...
def keypoint_metrics(
    keypoints_detected, keypoints_gt, all_json_np, image_resolution, auc_pixel_threshold=20.0, real=False
):
