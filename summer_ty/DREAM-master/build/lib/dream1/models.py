...
class DreamHourglass(nn.Module):
    def __init__(
        ...
        if self.deconv_decoder:
            ...
            self.upsample_0_4.add_module(
                "4", nn.Conv2d(640, 256, kernel_size=3, stride=1, padding=1)
            )
        ...
        self.mlp_x3d = nn.Sequential()
        self.mlp_x3d.add_module(
            '0', nn.Linear(21,64)
        )
        self.mlp_x3d.add_module("1", nn.ReLU(inplace=True))
        self.mlp_x3d.add_module(
            '2', nn.Linear(64, 128)
        )
    ...
    def forward(self, x, y = torch.zeros(1,7,3)):
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        x3d_1d = y.view(-1, 21).to(device)
        x3d_feature_128 = self.mlp_x3d(x3d_1d).unsqueeze(-1).unsqueeze(-1)
        x3d_feature_128_25_25 = x3d_feature_128.repeat(1,1,25,25)
        ...
        decoder_input = torch.cat((decoder_input, x3d_feature_128_25_25), dim = 1)
        # Decoder
        if self.deconv_decoder:
